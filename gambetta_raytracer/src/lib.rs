...
fn trace_ray(
    ...
    let (closest_sphere, closest_t) = closest_intersection(origin, direction, min_t, max_t, spheres);
    match closest_sphere {
        Some(sphere) => {
            ...
            let intensity = compute_lighting(point, normal, view, lights, sphere.specular, spheres);
            let local_color = common::multiply_color(intensity, sphere.color);
...
fn compute_light_from_direction(
    ...
    // shadow check
    let (closest_sphere, _) = closest_intersection(point, light_direction, 0.001, max_t, spheres);
    match closest_sphere {
        Some(_) => (),
        None => {
            // diffuse
